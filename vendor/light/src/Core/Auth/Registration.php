...
class Registration extends Auth
{
     ...
     */
    public function registration($username, $email, $password, $password_confirm, $first_name, $last_name, $rememberMe)
    {
        ...
        if (!$this->checkEmptyFieldsRegistration($this->data)) {
            if ($this->checkColumnMatch($this->data)) {
                ...
                $table[$this->db['columns']['remember_token']] = $rememberToken;
                R::store($table);
